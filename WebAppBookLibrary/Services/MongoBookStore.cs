...
public sealed class MongoBookStore : IBookStore
{
    ...
    public async Task<PagedResult<BookCatalogEntry>> SearchAsync(NormalizedBookQuery query, bool includeInactive, CancellationToken cancellationToken)
    {
        var builder = Builders<Book>.Filter;
        var filters = new List<FilterDefinition<Book>>();
        if (!includeInactive) filters.Add(builder.Eq(book => book.IsActive, true));
        ...
        if (query.MediaType is not null) filters.Add(builder.Eq(book => book.MediaType, query.MediaType));
        if (query.Language is not null) filters.Add(builder.Eq(book => book.Language, query.Language));
        if (query.Available is not null)
        {
            var available = builder.Or(builder.Eq(book => book.MediaType, MediaTypes.Digital), builder.Gt(book => book.AvailableCopies, 0));
        ...
        return new(books.Select(book => new BookCatalogEntry(book, counts.GetValueOrDefault(book.Id), false)).ToArray(), query.Page, query.PageSize, total);
    }

    public async Task<BookCatalogEntry?> FindCatalogEntryAsync(string id, bool includeInactive, CancellationToken cancellationToken)
    {
        var filter = Builders<Book>.Filter.Eq(book => book.Id, id);
        if (!includeInactive) filter &= Builders<Book>.Filter.Eq(book => book.IsActive, true);
    ...
    {
        var filter = Builders<Loan>.Filter.Eq(loan => loan.BookId, bookId) &
                     Builders<Loan>.Filter.Eq(loan => loan.IsReturned, false);
        return checked((int)await _loans.CountDocumentsAsync(filter, cancellationToken: cancellationToken));
    }
    public async Task<bool> ReplaceMetadataAsync(Book book, CancellationToken cancellationToken) => (await _books.ReplaceOneAsync(item => item.Id == book.Id, book, cancellationToken: cancellationToken)).MatchedCount == 1;
