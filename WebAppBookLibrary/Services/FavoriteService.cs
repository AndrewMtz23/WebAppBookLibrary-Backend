...
{
    public async Task<IReadOnlyList<FavoriteResponse>?> ListAsync(string username, CancellationToken token)
    {
        var user = await store.FindActiveUserAsync(username, token);
        if (user is null) return null;
        return (await store.ListAsync(user.Id, token)).Select(Map).ToArray();
    }
